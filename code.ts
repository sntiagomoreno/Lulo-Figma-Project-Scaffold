...
if (figma.command == "scaffold") {
  ...
  let run = async () => {
    ...
    await figma.loadFontAsync({ family: "Gilroy", style: "Medium" });
  ...
  let createLabel = (text: string, size: number, position: [number, number], color: object, uppercase?: boolean, space?: boolean, style: string = "Medium") => {
    ...
    fill = color
    label.fills = fill
    if (space) label.letterSpacing = { unit: "PIXELS", value: 1.69 }
    if (uppercase) label.textCase = "UPPER"
